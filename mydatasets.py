...
class GDELT(TarDataset):
    ...
    def __init__(self, text_field, label_field, path=None, examples=None, **kwargs):
        ...
        if examples is None:
            examples = []
            with open("content.theme.100w.new", errors='ignore') as f:
                line = f.readline()
                while line:
                    text = f.readline().strip()
                    label = f.readline().strip().replace(';', ' ')
                    if len(label) != 0:
                        if len(text.split(' ')) < 500:
                        #text = ' '.join(text.split(' ')[:500])
                            examples.append(data.Example.fromlist([text, label], fields))
                    line = f.readline()
        super(GDELT, self).__init__(examples, fields, **kwargs)
